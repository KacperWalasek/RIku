...
void processInput(GLFWwindow *window)
{
	...
	if(optCursorPos.has_value()) {
		...
		if (cursorPos.x < 0.03f) {
			...
			dx = -1.0f;
			dy = 1.0f - 2.0f * cursorPos.y;
		} else if (cursorPos.y < 0.05f) {
			dy = 1.0f;
			dx = 1.0f - 2.0f * cursorPos.x;
		} else if (cursorPos.y > 0.95f) {
			dy = -1.0f;
			dx = 1.0f - 2.0f * cursorPos.x;
		}
	}
	...
	for(int i=0;i<map.size();i++)
	{
		for(int j=0;j<map[i].size();j++)
		{
			...
			if(map[i][j].object && front::objectModels.count(map[i][j].object->getName())) {
				front::Object mapObject = front::Object(front::objectModels[map[i][j].object->getName()],glm::vec3(i,(float)map[i][j].height,j),glm::vec3(-90.0f,0.0f,180.0f),glm::vec3(0.1f,0.1f,0.1f));
...
int main() {
	...
	//objects
	front::objectModels.insert(std::make_pair("wood_factory",Model("models/objects/sawmill.blend",1,-1)));
	//units
