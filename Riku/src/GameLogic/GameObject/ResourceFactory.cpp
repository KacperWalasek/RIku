...
std::shared_ptr<IMove> ResourceFactory::onTurnEnd(int mapX, int mapY)
{
	if (mapX < 0 || mapY < 0) // fabryka nie stoi na ¿adnym polu
		return TileObjectDecorator::onTurnEnd(mapX, mapY);
	// TODO: player index
	auto extract = std::make_shared<ExtractResources>(getOwner(), resource, quantity, mapX, mapY, resource);
	return std::make_shared<CombinedMove>(extract, TileObjectDecorator::onTurnEnd(mapX, mapY));
}
