...
ChoseGuiOptionMoveHandler::ChoseGuiOptionMoveHandler(const GameState& state, const LogicAssets& assets)
...
std::shared_ptr<IMove> ChoseGuiOptionMoveHandler::handleDescription(const IMoveDescription& description)
{
    ...
    return state.map[desc.getMapX()][desc.getMapY()].object->onOptionChosen(desc.getIndex(), assets);
}
